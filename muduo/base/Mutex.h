...
public:
  ...
  void lock() ACQUIRE() {
    ...
    assignHolder(); /* 填充拥有者为当前线程id */
  }

  void unlock() RELEASE() {
    unassignHolder(); /* 删除拥有者线程id */
...
private:
  ...
  /* 可以看到UnassignGuard是用来清除拥有者线程id的，
   *在Condition的wait过程中会解锁，这意味着此时我们
   *可以在cond_wait之前清除拥有者线程信息，等到cond_wait结束后
   *UnassignGuard对象析构，将锁的拥有线程信息重新填充。
   */
  ...
  void assignHolder() /* 设置拥有者 */
  {
    holder_ = CurrentThread::tid();
  }

  pthread_mutex_t mutex_; /* 锁的核心 */
